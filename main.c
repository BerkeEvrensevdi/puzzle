...
int main()
{
   ...
   Word_t *arr =(Word_t*)malloc(words*sizeof(Word_t));


   arr = loadTextFile(fp,words);
   ...
   playGame(board,words,arr, rows, columns, 0 , words);
...
char line[150];
    ...
    (*(array+i)).word = (char*)malloc(256*sizeof(char));
    (*(array+i)).clue = (char*)malloc(256*sizeof(char));
    fscanf(myFile,"%c %d %d %s %[^\n]\n",&(*(array+i)).direction,&(*(array+i)).x,&(*(array+i)).y,(*(array+i)).word,(*(array+i)).clue);
                ...
                for( int t = 0; t < length; t++){
                    for( int k = 0; k < strlen((*(words+select-1)).word); k++){
                        if ((*(words+select-1)).y + k == (*(words+t)).y && (*(words+t)).direction == 'V' && ((*(words+select-1)).x >= (*(words+t)).x) && ((*(words+select-1)).x <= ((*(words+t)).x + strlen((*(words+t)).word)))) {
                            //printf("\nx = %d and y = %d", (*(words+t)).x,(*(words+t)).y);
                            int flag = 1;
